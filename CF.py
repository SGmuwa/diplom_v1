...
class CF:
    ...
    def build(self, ratings):
        ...
        ratings["avg"] = ratings.groupby(
            "user_id")["rating"].transform(lambda x: self.normalize(x))
...
ratings_df = pd.read_csv('ml-100k/u.data', names=[
                         'user_id', 'movie_id', 'rating', 'timestamp'], sep='\t', encoding='latin-1', header=None)
...
user_rated_movies = list(
    ratings_df[ratings_df["user_id"] == test_id]["movie_id"])
print(user_rated_movies)
movies[movies["movie_id"].isin(user_rated_movies)].iloc[:, [
    0, 1, 2, 10, 11, 13, 19, 21, 22, 23]]
...
for i, el in enumerate(user_rated_movies):
    ...
    dict_ = {i+1: res[i]
             for i in np.nonzero(res)[0] if i+1 not in user_rated_movies}
...
for i, el in enumerate(candidate_items):
    ...
    res = similarity_table[el-1, :]
    sim_items = {
        i+1: res[i] for i in np.nonzero(res)[0] if i+1 != el and i+1 in user_rated_movies}
    pre = 0
    if(len(sim_items) > 1):  # Желательно иметь более одного оцененного элемента
        mean_ui_df = ratings_df.loc[(ratings_df['user_id'] == test_id) & (
            ratings_df['movie_id'].isin(sim_items.keys()))]
#         print(el)
#         print(sim_items)
#         print(mean_ui_df)
...
sorted_items = sorted(rec.items(), key=lambda item: -
                      float(item[1]["prediction"]))[:10]
...
def recommend_item_by_ratings(user_id, active_user_items, num=6):
    ...
    movie_ids = dict(
        zip(list(active_user_items["movie_id"]), list(active_user_items["rating"])))
    # Расчёт среднего
